			...
			<div class="content_right" id="personal_detail_content">
				...
				<div class="per_detail_content">
					<form name="personForm" method="post"
						...
						<ul>
							...
							</label> <input type="text" name="phone" placeholder="请输入手机号码"
							...
							<li><label> 地区： </label> <select id="province"
								name="city" class="cityclass" onchange="checkCity()"
								onBlur="checkSelect()">
									<c:choose>
										<c:when test="${!empty user.city }">
											<option value='${user.city}'>${city.name}</option>
										</c:when>
										<c:otherwise>
									...
									<c:forEach items="${provinces }" var="province">
										...
										${city.parentId==province.id?'selected':''}>${province.name}</option>
									</c:forEach>
							</select> <select id="select_city" name="country" class="city cityclass"
				...
				<div class="head_detail_content">
					<form name="headForm" method="post"
						action="/personal/CropAction?pictureType=headPic"
