...
<body>
	...
	<div class="content_infor">
            <a href="/index">我的去哪玩</a>> <a
                href="/personal/personalinf">个人空间</a>> <a id="target_title3"
	...
	<div class="content_left_first">
        <c:choose>
            ...
            <c:otherwise>
                <img src="img/user_img/${user.imgPath}"
                     class="img_head" />
            </c:otherwise>
        </c:choose>
		<p>编辑我的资料</p>
	...
	<div class="content_left_two">
		<ul>
			...
			<li ><a
					href="${pageContext.request.contextPath }/personal/myContacts"><span
		...
		<div class="content_right_pa">
			...
			<div class="content_right bordernone" id="my_re_list_content">
				...
				<div class="re_my_detail">
					...
					<input type="hidden" id="length" value="${fn:length(bigsb) }">
							...
							<c:forEach items="${orderFormList}" var="item" varStatus="varStatus">
								<tr class="tr_width margin-right5  text_left">
									<td colspan="5">
										...
										<span class="text_middle margin-right15">${item.createTime}</span>
									</td>
								</tr>
								<tr>
									<td>
									...
									<td class="text_color_orange">${item.totalPrice}</td>
									<td>${item.num}</td>
									<td>
										<span class="text_color span_width ">
											<a class="text_color" href=
												<c:if test="${ item.sequence.keying == 0}">
													'go_pay?type=showPayFor&id=${ item.id}'
												</c:if>  
												<c:if test="${ item.sequence.keying == 1}">
													'#'
												</c:if> 												
												<c:if test="${ item.sequence.keying == 3}">
													"/personal/myComments"
												</c:if>>
												<c:if test="${ item.sequence.keying == 0}">
													付款
										...
										<span class="text_color span_width"><a  href="myorder?type=initOrderDetail&order_id=${ item.id}" target="view_window">订单详情</a></span>
									</td>
									<td><span class="text_color span_width  ">
												<c:if test="${ item.sequence.value=='待评价'}">
													<a title="为了方便测试,付款后直接进入待评价状态" href="javascript:;">
												</c:if>
												<c:if test="${ item.sequence.value!='待评价'}">
													<a href="javascript:;">
												</c:if>${item.sequence.value}</a>
										</span> <br />
										<span class="text_color_blue " id="${ item.orderno}"></span>
										<input type="hidden" value="${item.orderno }" id="timeid${varStatus.index }">
										<input type="hidden" value="${item.startTime}" id="start_time${varStatus.index }">
										<input type="hidden" value="${item.createTime }" id="create_time${varStatus.index }">
										<input type="hidden" value="${item.sequence.keying }" id="keying${varStatus.index }">
										<input type="hidden" value="${item.totalPrice }" id="totaldays${varStatus.index }">
										<span id="time"></span>
									</td>
								</tr>
								<tr class="sep-row">
					...
					<c:if test="${cur!=1 }">
						<a href="/personal/myOrder?page=${cur-1}" title="上一页" class="PrevPage">上一页</a>
					</c:if>
					<c:if test="${cur==1 }">
					...
					<c:forEach var="i" begin="1" end="${pageCount}">
						<c:if test="${i==cur}">
								<a href="/personal/myOrder?page=${i}" class="pagesel" style="background-color:#DF1A7A;
							color:white" id="orderPage" onclick="cur(${i})">${i}</a>
						</c:if>
						<c:if test="${i!=cur}">
								<a href="/personal/myOrder?page=${i}" class="pagesel"
							 id="orderPage" onclick="cur(${i})">${i}</a>
						</c:if>
					</c:forEach>
					<c:if test="${cur!=pageCount }">
						<a href="/personal/myOrder?&page=${cur+1}" title="下一页" class="NextPage" onclick="next()">下一页</a>
					</c:if>
					<c:if test="${cur==pageCount }">
