...
<body>
	...
	<div class="content_infor">
		<a href="/index">我的去哪玩</a>> <a
			href="/personal/personalinf">个人空间</a>>
			...
			<div class="content_right bordernone " id="usually_tourist_content">
				...
				<div class="tourist">
					<form id="contactForm" name="contactForm" action="" method="post">
						<ul>
							<input style="display: none" type="text" placeholder="请输入中文姓名" name="c_id"
								   id="cid" class="input_style" />
						...
						<thead>
							...
							<c:forEach items="${formList}" var="value" >
								<tr id='${ value.id}'>
									...
									<td>${value.cardno}</td>
									<td>${value.email}</td>
									<c:if test="${value.sex==1 }">
										<td>男</td>
									</c:if>
									<c:if test="${value.sex==0 }">
										<td>女</td>
									</c:if>
				...
				<div class="Pages" id="contactPages" >
					<c:if test="${curc>1 }">
						<a href="/contact/show?page=${curc-1}" title="上一页" class="PrevPage">上一页</a>
					</c:if>
					<c:if test="${curc==1 }">
						<a href="javascript:;" title="首页" class="PrevPage">首页</a>
					</c:if>
					
					<c:forEach var="i" begin="1" end="${pageCountc}">
						<c:if test="${i==curc}">
					...
					<c:if test="${curc!=pageCountc }">
						<a href="/contact/show?page=${curc+1}" title="下一页" class="NextPage" onclick="next()">下一页</a>
					</c:if>
					<c:if test="${curc==pageCountc }">
						<a href="javascript:;" title="${curc }和${pageCountc}尾页" class="NextPage" onclick="next()" >尾页</a>
					</c:if>
				</div>
