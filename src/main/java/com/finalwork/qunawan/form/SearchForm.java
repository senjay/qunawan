...
public class SearchForm {
	...
	public void setTripList(List<Trip> tripList) {
		...
		for (Trip t : tripList) {
			...
			// 详情页trim函数场景测试用
			sequence.setValue("  " + t.getSeqType().getValue());
			...
			for (Themeontrip tot : t.getThemeontrip_list()) {
				...
				Theme th = new Theme();
				th.setName(tot.getThemeObject().getName());
				new_tot.setThemeObject(th);
			...
			for (Placeontrip pot : t.getPlaceontrip_list()) {
				...
				Place p = new Place();
				p.setName(pot.getPlaceObject().getName());
				new_pot.setPlaceObject(p);
