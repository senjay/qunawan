package com.finalwork.qunawan.bean;
	...
	private List<String> getCurThemeList(List<Trip> tripList) {
		...
		for (Trip t : tripList)
			for (Themeontrip tot : t.getThemeontrip_list())
				if (!list.contains(tot.getThemeObject().getName()))
					list.add(tot.getThemeObject().getName());
		return list;
	}
	...
	private List<String> getCurPlaceList(List<Trip> tripList) {
		...
		for (Trip t : tripList)
			for (Placeontrip pot : t.getPlaceontrip_list())
				if (!list.contains(pot.getPlaceObject().getName()))
					list.add(pot.getPlaceObject().getName());
		return list;
	}
}
